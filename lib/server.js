...
function createServer (config, callback) {
  ...
  extension.loadConfiguration(config, function (err) {
    ...
    const activeLogger = config.winstonInstance ? config.winstonInstance : logger
    ...
    server = app.listen(port, function () {
      activeLogger.info('Express integrator-extension server listening on port: ' + port)
      return callback()
    })
    ...
    server.on('error', function (err) {
      activeLogger.error('Express integrator-extension server error - ' + err.toString())
    })
...
function stopServer (callback) {
  ...
  server.close(function (err) {
    ...
    activeLogger.info('Express integrator-extension server stopped listening on port: ' + port)
    return callback()
  })
}
